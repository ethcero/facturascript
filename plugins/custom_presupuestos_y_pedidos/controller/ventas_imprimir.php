    ...
    protected function generar_pdf_lineas_venta(&$pdf_doc, &$lineas, &$linea_actual, &$lppag)
    {
        ...
        for ($i = $linea_actual; (($linea_actual < ($lppag + $i)) && ( $linea_actual < count($lineas)));) {
            ...
            if(isset($_GET['solototal'])){
                $fila['pvp'] = $this->show_precio($lineas[$linea_actual]->pvpunitario * ($lineas[$linea_actual]->iva /100), $this->documento->coddivisa, TRUE, FS_NF0_ART);
                $fila['importe'] = $this->show_precio($lineas[$linea_actual]->pvptotal * ($lineas[$linea_actual]->iva /100), $this->documento->coddivisa);
            }
